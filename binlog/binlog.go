...
func (b *Binlog) Encode() []byte {
	...
	// binlog type
	copy(data[5:6], []byte(fmt.Sprintf("%v", b.Tp)))
	...
	binary.LittleEndian.PutUint32(data[offset:offset+4], uint32(preWriteKeyLen))
	offset += 4
	...
	// crc
	crc := crc32.Checksum(data[5:dataSize-4], crcTable)
